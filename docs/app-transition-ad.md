...
앱 전환 광고는 앱 내에서 실행 또는 화면 전환 시 노출되는 팝업 광고로 사용자 흐름에 자연스럽게 삽입됩니다.<br/>
매체의 니즈에 따라 이미지 크기, 프로필 유무, 광고 위치를 선택하여 광고를 구성할 수 있습니다.

앱 전환 광고를 시작하기 위해 먼저 카카오애드핏 플랫폼에서 자신의 매체를 등록한 후, 앱 전환 광고유형의 광고단위 ID를 발급받아야 합니다.
...
앱 전환 광고는 `AdFitPopupAdLoader`와 `AdFitPopupAdRequest`를 사용하여 요청할 수 있습니다.
...
`AdFitPopupAdLoader`가 준비되면, 앱 전환 광고 노출을 위한 적절한 시점에 `loadAd()` 메서드를 호출하여 광고를 요청합니다.

```kotlin
// 앱 전환 광고 요청 시작
popupAdLoader.loadAd(
    AdFitPopupAdRequest.build(AdFitPopupAd.Type.Transition /* 팝업 광고 타입: 앱 전환 */),
...
* 광고 요청 정책 확인: `isBlockedByRequestPolicy` 프로퍼티를 통해 "오늘 그만보기" 선택 여부나 빈도 제한 정책에 따라 요청이 제한된 상태인지 확인합니다.
...
* 요청 실패: 요청에 실패하거나 표시할 광고가 없으면, 오류 코드(`errorCode`)와 함께 `onAdLoadError()` 콜백이 호출됩니다. <br/>
  ...
  | 301  | 요청에는 성공했으나 앱 전환 광고 유형에 맞는 소재가 없는 경우 |
...
이 함수는 파라미터로 `AdFitPopupAd` 객체를 전달받으며, `AdFitPopupAdDialogFragment`에 설정하여 앱 전환 광고를 팝업 형식으로 화면에 표시할 수 있습니다.
    ...
    supportFragmentManager.setFragmentResultListener(
        ...
        when (bundle.getString(AdFitPopupAdDialogFragment.BUNDLE_KEY_EVENT_TYPE)) {
            ...
            AdFitPopupAdDialogFragment.EVENT_POPUP_DISMISSED -> {
                // TODO: "닫기"를 선택한 경우에 대한 처리
            }

            AdFitPopupAdDialogFragment.EVENT_TODAY_DISMISSED -> {
                // TODO: "오늘 그만보기"를 선택한 경우에 대한 처리
            }
        }
    }
...
| EVENT_POPUP_CANCELED      | 사용자가 '뒤로 가기' 버튼을 눌러 팝업을 취소했을 때 발생합니다.           |
| EVENT_POPUP_DISMISSED     | 사용자가 팝업의 '닫기' 버튼을 눌러 팝업을 닫았을 때 발생합니다.          |
| EVENT_TODAY_DISMISSED     | 사용자가 '오늘 그만보기' 버튼을 눌러 팝업을 닫았을 때 발생합니다.         |


## 테스트를 위한 광고 요청 제한 초기화

앱 전환 광고는 사용자의 피로도 감소와 경험 품질 유지를 위해 "오늘 그만보기"나 내부적인 빈도 제한(Frequency Capping)과 같은 요청 제한 정책을 가지고 있습니다.
...
resetButton.setOnClickListener {
    /**
     * "오늘 그만보기"나 빈도 제한 등으로 인해 광고 요청이 불가능한 상태일 때,
